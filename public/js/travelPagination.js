...
function travelpageRender(data) {
  ...
  data.forEach(function(item) {
    ...
    if (urlString === item.ID) {
      let travelpageLodingShow = document.querySelector(
        ".travelpage-loding-show"
      );
        ...
        .onSnapshot(function(snapshop) {
          ...
          function renderMessage() {
            for (let i = 0; i < len; i++) {
              ...
              let travelpageMessage = document.querySelector(
                ".travelpage-message"
              );
              let travelpageMessageContent = document.createElement("div");
              travelpageMessageContent.setAttribute(
                "class",
                "travelpage-message-content"
              );

              let travelpageMessageContentRight = document.createElement("div");
              travelpageMessageContentRight.setAttribute(
                "class",
                "travelpage-message-content-right"
              );
              ...
              travelpageMessageContentLeft.setAttribute(
                "class",
                "travelpage-message-content-left"
              );

              let travelpageMessageTitle = document.createElement("div");
              travelpageMessageTitle.setAttribute(
                "class",
                "travelpage-message-title"
              );

              let travelpageMessageName = document.createElement("div");
              travelpageMessageName.setAttribute(
                "class",
                "travelpage-message-name"
              );
              travelpageMessageName.textContent = name;

              let travelpageMessageTime = document.createElement("div");
              travelpageMessageTime.setAttribute(
                "class",
                "travelpage-message-time"
              );
              travelpageMessageTime.textContent = time;

              let travelpageMessageMemo = document.createElement("div");
              travelpageMessageMemo.setAttribute(
                "class",
                "travelpage-message-memo"
              );
              travelpageMessageMemo.textContent = text;

              //留言版功能
              travelpageMessage.appendChild(travelpageMessageContent);
              travelpageMessageContent.appendChild(
                travelpageMessageContentLeft
              );
              travelpageMessageContent.appendChild(
                travelpageMessageContentRight
              );
...
let travelpageMessageComment = document.querySelector(
